...
export async function GET(_: NextRequest, context: { params: Promise<{ investigationId: string }> }) {
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/api/v1/investigations/${investigationId}/validation-summary`, {
      headers: { authorization: `Bearer ${token}` },
      cache: "no-store",
    });
    return new NextResponse(await response.text(), { status: response.status, headers: { "content-type": "application/json" } });
  } catch {
    return NextResponse.json({ detail: "Validation summary service is unavailable" }, { status: 503 });
  }
}
