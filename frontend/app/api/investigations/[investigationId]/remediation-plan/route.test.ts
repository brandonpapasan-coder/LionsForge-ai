...
import { GET } from "@/app/api/investigations/[investigationId]/remediation-plan/route";

const context = { params: Promise.resolve({ investigationId: "7" }) };

describe("evidence gap remediation API route", () => {
  beforeEach(() => {
    cookieGet.mockReset();
    vi.unstubAllGlobals();
  });
  ...
  it("forwards authenticated requests with no-store", async () => {
    ...
    const payload = { status: "complete", actions: [] };
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify(payload), { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
    const response = await GET(new Request("http://localhost/api") as never, context);
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8000/api/v1/investigations/7/remediation-plan",
  ...
  it("returns a stable 503 without leaking backend details", async () => {
    ...
    expect(payload).toEqual({ detail: "Evidence gap remediation service is unavailable" });
