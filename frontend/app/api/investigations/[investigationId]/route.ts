...
async function proxy(method: "GET" | "PATCH", request: NextRequest | undefined, context: RouteContext) {
  const token = (await cookies()).get("lionsforge_session")?.value;
  if (!token) return NextResponse.json({ detail: "Not authenticated" }, { status: 401 });
  const { investigationId } = await context.params;

  try {
    const response = await fetch(`${backendUrl}/api/v1/investigations/${investigationId}`, {
      method,
      headers: {
