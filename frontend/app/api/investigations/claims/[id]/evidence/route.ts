...
async function proxy(method: "GET" | "POST", request: NextRequest | undefined, id: string) {
  ...
  try {
    const response = await fetch(`${backendUrl}/api/v1/investigations/claims/${id}/evidence`, {
      method,
    ...
    return NextResponse.json({ detail: "Evidence service is unavailable" }, { status: 503 });
  }
}
