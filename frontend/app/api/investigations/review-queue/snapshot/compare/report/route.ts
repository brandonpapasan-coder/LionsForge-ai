...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/api/v1/investigations/review-queue/snapshot/compare/report`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Review queue comparison report export is unavailable" }, { status: 503 });
  }
}
