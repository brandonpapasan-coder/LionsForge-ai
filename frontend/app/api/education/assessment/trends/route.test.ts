...
import { GET } from "@/app/api/education/assessment/trends/route";

describe("education competency trends API route", () => {
  beforeEach(() => {
    cookieGet.mockReset();
    vi.unstubAllGlobals();
  });
  ...
  it("forwards authenticated trend requests with no-store", async () => {
    cookieGet.mockReturnValue({ value: "session-token" });
    const trends = [{ competency: "valuation", direction: "stable" }];
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8000/api/v1/education/assessment/trends",
      {
    ...
    expect(await response.json()).toEqual(trends);
  });
  ...
  it("returns a stable 503 without leaking backend details", async () => {
    ...
    expect(payload).toEqual({ detail: "Education competency trend service is unavailable" });
