...
import { GET } from "@/app/api/knowledge-quality/projects/[projectId]/route";

function context(projectId: string) {
  return { params: Promise.resolve({ projectId }) };
}

describe("project knowledge-quality API proxy", () => {
  beforeEach(() => {
    getCookie.mockReset();
    vi.restoreAllMocks();
  });
  ...
  it("rejects invalid project identifiers without calling the backend", async () => {
    getCookie.mockReturnValue({ value: "session-token" });
    const fetchMock = vi.spyOn(globalThis, "fetch");

    const response = await GET(new Request("http://localhost"), context("7-invalid"));
  ...
  it("forwards the bearer token and preserves an upstream response", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ project_id: 7, health_score: 0.91 }), {
        status: 200,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8000/api/v1/knowledge-quality/projects/7",
      {
    ...
    await expect(response.json()).resolves.toEqual({ project_id: 7, health_score: 0.91 });
  });
  ...
  it("preserves upstream error status and body", async () => {
    ...
    const response = await GET(new Request("http://localhost"), context("99"));
