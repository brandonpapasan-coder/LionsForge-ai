...
const packet = JSON.stringify({ content_sha256: "a".repeat(64), content: { schema_version: "1.0", title: "Earlier" } });
...
test("validates both packet inputs", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Create comparison report" }));
...
test("creates and displays a comparison report", async () => {
  ...
  render(<ResearchPacketComparisonReportWorkspace />);
  fireEvent.change(screen.getByLabelText("Earlier packet JSON"), { target: { value: packet } });
  fireEvent.change(screen.getByLabelText("Later packet JSON"), { target: { value: packet } });
  fireEvent.click(screen.getByRole("button", { name: "Create comparison report" }));

  expect(await screen.findByText("Report status: different")).toBeInTheDocument();
  expect(screen.getByText("title")).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "Download JSON report" })).toBeInTheDocument();
  expect(global.fetch).toHaveBeenCalledWith("/api/research-packet-comparison-report", expect.objectContaining({ method: "POST" }));
});

test("loads a local JSON packet file", async () => {
  render(<ResearchPacketComparisonReportWorkspace />);
  const file = new File([packet], "packet.json", { type: "application/json" });
  fireEvent.change(screen.getByLabelText("Load earlier JSON"), { target: { files: [file] } });
  await waitFor(() => expect(screen.getByLabelText("Earlier packet JSON")).toHaveValue(packet));
});
