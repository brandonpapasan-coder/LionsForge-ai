...
export function ResearchPacketIntegrityChecker() {
  ...
  return <section className="research-provenance-section">
    <div className="research-section-heading"><div><p className="eyebrow">RESEARCH WORKSPACE</p><h1>Packet integrity checker</h1><p className="muted">Recompute an exported packet&apos;s canonical SHA-256 value and compare it with the supplied value.</p></div></div>
    <section className="dashboard-panel">
      <label>Packet JSON<textarea aria-label="Packet JSON" rows={18} value={text} onChange={(event) => setText(event.target.value)} placeholder="Paste a LionsForge research packet here" /></label>
      <label>Load JSON packet<input aria-label="Load JSON packet" type="file" accept="application/json,.json" onChange={(event) => void loadFile(event)} /></label>
      <button type="button" disabled={checking} onClick={() => void verify()}>{checking ? "Checking…" : "Check integrity"}</button>
    </section>
    {error ? <p role="alert" className="form-message">{error}</p> : null}
    {result ? <section className="dashboard-panel" aria-label="Integrity result">
