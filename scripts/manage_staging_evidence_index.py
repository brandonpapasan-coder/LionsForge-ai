...
SCHEMA = "lionsforge.staging-evidence-index"
RECEIPT_SCHEMA = "lionsforge.staging-evidence-index-receipt"
SCHEMA_VERSION = 1
...
NOTICE = (
    "This index binds supplied repository and staging evidence only. It does not provision infrastructure, "
...
def validate_generation_input(value: Any) -> list[str]:
    ...
    required = {"candidate_sha", "selection_rationale", "generated_at", "entries"}
...
def _entry_findings(entries: Any, candidate_sha: str | None) -> tuple[list[str], bool]:
    if not isinstance(entries, list) or len(entries) != len(REQUIRED_ENTRIES):
        return ["entries must contain exactly the required staging evidence categories"], False
    findings: list[str] = []
    ready = True
    ...
    for item in entries:
        ...
        elif category == "staging_acceptance_record":
            if item["decision"] != "GO":
                ready = False
...
def build_index(*, candidate_sha: str, selection_rationale: str, entries: list[dict[str, Any]], generated_at: datetime) -> dict[str, Any]:
    ordered = sorted(entries, key=lambda item: item.get("category", ""))
    structural, ready = _entry_findings(ordered, candidate_sha)
    index = {
        "schema": SCHEMA,
    ...
    if findings:
        raise ValueError("Invalid staging evidence index: " + "; ".join(findings))
    return index
...
def validate_index(value: Any) -> list[str]:
    ...
    structural, ready = _entry_findings(value.get("entries"), sha if isinstance(sha, str) else None)
...
def validate_bundle(value: Any) -> list[str]:
    ...
    required = {"schema", "schema_version", "generator_version", "index_sha256", "candidate_sha", "decision", "generated_at"}
