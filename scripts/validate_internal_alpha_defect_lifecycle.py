...
ITEM = {"defect_id", "feedback_id", "tester_id", "session_id", "release_candidate", "severity", "previous_severity", "state", "previous_state", "regression", "owner_ref", "reason_codes", "created_at", "updated_at", "verified_at"}
...
def validate(value: object, expected_candidate: str | None = None, *, now: datetime | None = None) -> dict[str, object]:
    ...
    for item in defects:
        ...
        if not isinstance(reasons, list) or not reasons or len(reasons) > 8 or len(set(reasons)) != len(reasons):
        ...
        if SEVERITIES[severity] < SEVERITIES[previous_severity] and "severity-downgrade-approved" not in reasons:
