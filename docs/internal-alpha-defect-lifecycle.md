...
A successful result preserves `INTERNAL-ALPHA-ONLY`. It validates and summarizes defect records only; it does not expose defects publicly, create accounts or credentials, enable external beta, deploy software, collect payments, enable production billing, or authorize general availability.
