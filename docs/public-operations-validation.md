...
The persisted artifact is then independently schema-validated before it may be summarized or uploaded. The validator performs a bounded no-follow read and rejects empty files, oversized files, symbolic links, non-regular files, invalid UTF-8, malformed JSON, duplicate JSON object keys at any nesting level, replacement races, and truncation or mutation during reading. It accepts only recognized GitHub Actions statuses and conclusions, requires conclusions to remain null before completion, requires missing gates to contain no run metadata, binds every real run URL exactly to the requested repository and run ID, and requires each non-missing gate to use a distinct run ID and distinct canonical run URL. Repository and release-SHA mismatches, altered workflow maps, reordered or spoofed gates, invalid event or branch values, cross-repository URLs, URL/run-ID mismatches, reused run identities, ambiguous repeated keys, and a top-level `passed` value that disagrees with the gate evidence are blocking failures. A structurally valid failure artifact may be retained for diagnosis, but it does not satisfy the release gates.
