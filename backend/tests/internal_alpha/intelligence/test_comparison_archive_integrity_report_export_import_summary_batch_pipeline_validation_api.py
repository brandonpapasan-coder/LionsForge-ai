...
def _payload() -> dict:
    return {"summaries": [{"summary": 1}], "pipeline": _pipeline()}
...
def test_batch_pipeline_validation_api_requires_authentication():
    app = FastAPI()
    app.include_router(router)
...
def test_batch_pipeline_validation_api_forwards_exact_inputs(monkeypatch):
    captured = {}
...
def test_batch_pipeline_validation_api_returns_invalid_findings(monkeypatch):
    monkeypatch.setattr(
        "app.api.routes."
        "internal_alpha_comparison_archive_integrity_report_export_import_summary_batch_pipeline."
        "validate_intelligence_comparison_archive_integrity_report_export_import_summary_batch_pipeline",
        lambda summaries, pipeline: ["pipeline keys do not match the canonical shape"],
    )
    ...
    assert response.json() == {
        "valid": False,
        "findings": ["pipeline keys do not match the canonical shape"],
        "interpretation_notice": _NOTICE,
    }


def test_batch_pipeline_validation_request_rejects_extra_fields():
    payload = _payload()
...
def test_batch_pipeline_validation_request_rejects_nested_extra_fields():
    payload = _payload()
    payload["pipeline"]["diagnostics"]["unexpected"] = True
    response = TestClient(_authenticated_app()).post(_PATH, json=payload)
    assert response.status_code == 422


def test_batch_pipeline_validation_request_rejects_nested_wrong_types():
    payload = _payload()
    payload["pipeline"]["batch_result"]["summary_count"] = "1"
    response = TestClient(_authenticated_app()).post(_PATH, json=payload)
    assert response.status_code == 422


def test_batch_pipeline_validation_request_rejects_empty_summaries():
    payload = _payload()
    payload["summaries"] = []
    response = TestClient(_authenticated_app()).post(_PATH, json=payload)
    assert response.status_code == 422
...
def test_batch_pipeline_validation_request_accepts_100_summaries(monkeypatch):
    monkeypatch.setattr(
        "app.api.routes."
        "internal_alpha_comparison_archive_integrity_report_export_import_summary_batch_pipeline."
        "validate_intelligence_comparison_archive_integrity_report_export_import_summary_batch_pipeline",
...
def test_batch_pipeline_validation_request_rejects_more_than_100_summaries():
    payload = _payload()
...
def test_batch_pipeline_validation_api_is_registered_in_router_openapi():
    app = FastAPI()
