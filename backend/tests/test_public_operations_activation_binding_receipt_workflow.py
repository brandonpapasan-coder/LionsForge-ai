...
WORKFLOW = Path(__file__).resolve().parents[2] / ".github" / "workflows" / "public-operations-activation-binding-receipt.yml"
...
def test_workflow_rejects_unsafe_paths_and_supports_replay_ledger() -> None:
    value = text()
    assert '[[ "${RECEIPT_PATH}" != /* ]]' in value
    assert '[[ "${RECEIPT_PATH}" != *".."* ]]' in value
    assert 'test ! -L "${RECEIPT_PATH}"' in value
    assert "--ledger" in value
    assert "validate_public_operations_activation_receipt.py" in value
