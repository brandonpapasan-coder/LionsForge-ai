...
WORKFLOW = Path(__file__).resolve().parents[2] / ".github" / "workflows" / "internal-alpha-defect-lifecycle.yml"
...
def test_workflow_preserves_internal_only_boundary() -> None:
    value = text()
    assert '[[ "${MANIFEST_PATH}" != /* ]]' in value
    assert '[[ "${MANIFEST_PATH}" != *".."* ]]' in value
    assert 'test ! -L "${MANIFEST_PATH}"' in value
    assert "validate_internal_alpha_defect_lifecycle.py" in value
    assert "--expected-candidate" in value
    assert "retention-days: 90" in value
    assert "does not expose defects publicly" in value
