...
def test_rejects_transition_and_unverified_closure() -> None:
    ...
    defect["state"] = "accepted"
    defect["previous_state"] = "fixed"
    defect["verified_at"] = None
    with pytest.raises(ValueError, match="transition regression"):
        validate(value)
...
def test_rejects_unapproved_downgrade_and_sensitive_fields() -> None:
    value = build()
    value["defects"][0]["severity"] = "medium"
    with pytest.raises(ValueError, match="downgrade requires approval"):
        validate(value)

    value = build()
    value["defects"][0]["comment"] = "free form"
    with pytest.raises(ValueError, match="free-form key"):
        validate(value)
