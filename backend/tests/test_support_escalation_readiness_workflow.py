...
WORKFLOW = Path(__file__).resolve().parents[2] / ".github" / "workflows" / "support-escalation-readiness.yml"

def text() -> str: return WORKFLOW.read_text(encoding="utf-8")
...
def test_workflow_rejects_unsafe_paths_and_retains_report() -> None:
    ...
    assert "validate_support_escalation_readiness.py" in value
    assert "support-escalation-readiness-report.json" in value
    assert "retention-days: 90" in value
    assert "does not prove live channel availability" in value
