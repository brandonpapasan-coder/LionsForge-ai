...
WORKFLOW = Path(__file__).resolve().parents[2] / ".github" / "workflows" / "public-operations-checkpoint-ledger-witness.yml"
...
def test_workflow_rejects_unsafe_paths_and_preserves_boundary() -> None:
    value = text()
    assert '[[ "${WITNESS_PATH}" != /* ]]' in value
    assert '[[ "${WITNESS_PATH}" != *".."* ]]' in value
    assert 'test ! -L "${WITNESS_PATH}"' in value
    assert "validate_public_operations_checkpoint_ledger_witness.py" in value
