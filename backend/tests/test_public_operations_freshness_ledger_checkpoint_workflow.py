...
WORKFLOW = Path(__file__).resolve().parents[2] / ".github" / "workflows" / "public-operations-freshness-ledger-checkpoint.yml"
...
def test_workflow_rejects_unsafe_paths_and_preserves_boundary() -> None:
    value = text()
    assert '[[ "${CHECKPOINT_PATH}" != /* ]]' in value
    assert '[[ "${CHECKPOINT_PATH}" != *".."* ]]' in value
    assert 'test ! -L "${CHECKPOINT_PATH}"' in value
    assert "validate_public_operations_freshness_ledger_checkpoint.py" in value
