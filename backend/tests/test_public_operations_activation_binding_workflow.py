...
WORKFLOW = Path(__file__).resolve().parents[2] / ".github" / "workflows" / "public-operations-activation-binding.yml"
...
def test_workflow_rejects_unsafe_paths_and_retains_report() -> None:
    ...
    assert "validate_public_operations_activation_binding.py" in value
    assert "public-operations-activation-binding-report.json" in value
