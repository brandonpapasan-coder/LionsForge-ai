...
WORKFLOW = (
    ...
    / ".github"
    / "workflows"
    / "public-operations-evidence-manifest.yml"
)
...
def test_workflow_rejects_unsafe_paths_and_retains_report() -> None:
    ...
    assert "validate_public_operations_evidence_manifest.py" in value
    assert "public-operations-evidence-report.json" in value
    assert "retention-days: 90" in value
    assert "binds file bytes and decisions only" in value
