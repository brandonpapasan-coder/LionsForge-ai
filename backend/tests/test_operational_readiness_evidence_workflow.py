...
WORKFLOW = Path(__file__).resolve().parents[2] / ".github" / "workflows" / "operational-readiness-evidence.yml"
...
def test_workflow_rejects_unsafe_paths_and_preserves_authorization_boundary() -> None:
    value = text()
    assert '[[ "${MANIFEST_PATH}" != /* ]]' in value
    assert '[[ "${MANIFEST_PATH}" != *".."* ]]' in value
    assert 'test ! -L "${MANIFEST_PATH}"' in value
    assert "validate_operational_readiness_evidence.py" in value
