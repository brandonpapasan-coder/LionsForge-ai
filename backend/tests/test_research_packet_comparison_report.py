import hashlib
import json

from fastapi.testclient import TestClient

from tests.conftest import auth_headers

ENDPOINT = "/api/v1/research-packet-comparison-report/export"


def canonical_sha256(content: dict[str, object]) -> str:
    payload = json.dumps(
        content,
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()


def packet(
    content: dict[str, object],
    supplied_hash: str | None = None,
) -> dict[str, object]:
    return {
        "content_sha256": supplied_hash or canonical_sha256(content),
        "content": content,
    }


def report_sha256(content: dict[str, object]) -> str:
    payload = json.dumps(
        content,
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()


def test_requires_authentication(client: TestClient):
    content = {"schema_version": "1.0"}
    response = client.post(
        ENDPOINT,
        json={"left": packet(content), "right": packet(content)},
    )
    assert response.status_code == 401


def test_exports_deterministic_identical_report(client: TestClient):
    headers = auth_headers(client)
    left = {"schema_version": "1.0", "nested": {"b": 2, "a": 1}}
    right = {"nested": {"a": 1, "b": 2}, "schema_version": "1.0"}
    response = client.post(
        ENDPOINT,
        headers=headers,
        json={"left": packet(left), "right": packet(right)},
    )
    body = response.json()
    assert response.status_code == 200
    assert body["content"]["status"] == "identical"
    assert body["content"]["differences"] == []
    assert body["content"]["left_hash_matches"] is True
    assert body["content"]["right_hash_matches"] is True
    assert body["report_sha256"] == report_sha256(body["content"])


def test_exports_added_removed_and_changed_paths(client: TestClient):
    headers = auth_headers(client)
    left = {
        "schema_version": "1.0",
        "title": "Old",
        "items": [1, 2],
        "removed": True,
    }
    right = {
        "schema_version": "1.0",
        "title": "New",
        "items": [1, 3, 4],
        "added": True,
    }
    response = client.post(
        ENDPOINT,
        headers=headers,
        json={"left": packet(left), "right": packet(right)},
    )
    body = response.json()
    assert response.status_code == 200
    assert body["content"]["status"] == "different"
    assert body["content"]["differences"] == [
        {"path": "added", "kind": "added"},
        {"path": "items[1]", "kind": "changed"},
        {"path": "items[2]", "kind": "added"},
        {"path": "removed", "kind": "removed"},
        {"path": "title", "kind": "changed"},
    ]
    assert body["content"]["added_count"] == 2
    assert body["content"]["removed_count"] == 1
    assert body["content"]["changed_count"] == 2
    assert body["report_sha256"] == report_sha256(body["content"])


def test_records_hash_mismatch_and_unsupported_schema(client: TestClient):
    headers = auth_headers(client)
    left = {"schema_version": "1.0", "value": 1}
    right = {"schema_version": "2.0", "value": 2}
    response = client.post(
        ENDPOINT,
        headers=headers,
        json={"left": packet(left, "0" * 64), "right": packet(right)},
    )
    body = response.json()
    assert response.status_code == 200
    assert body["content"]["status"] == "unsupported"
    assert body["content"]["left_hash_matches"] is False
    assert body["content"]["right_hash_matches"] is True
    assert body["content"]["right_schema_version"] == "2.0"
    assert body["content"]["differences"] == []
