...
WORKFLOW = Path(__file__).resolve().parents[2] / ".github" / "workflows" / "public-operations-checkpoint-ledger-witness-ledger.yml"
...
def test_workflow_rejects_unsafe_paths_and_preserves_boundary() -> None:
    value = text()
    assert '[[ "${LEDGER_PATH}" != /* ]]' in value
    assert '[[ "${LEDGER_PATH}" != *".."* ]]' in value
    assert 'test ! -L "${LEDGER_PATH}"' in value
    assert "validate_public_operations_checkpoint_ledger_witness_ledger.py" in value
