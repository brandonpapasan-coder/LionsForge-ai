...
def test_workflow_rejects_unsafe_paths_and_retains_report() -> None:
    ...
    assert "validate_privacy_request_readiness.py" in value
    assert "privacy-request-readiness-report.json" in value
    assert "retention-days: 90" in value
    assert "does not provide legal advice" in value
