...
WORKFLOW = Path(__file__).resolve().parents[2] / ".github" / "workflows" / "internal-alpha-tester-access.yml"
...
def test_workflow_preserves_internal_only_boundary() -> None:
    ...
    assert "validate_internal_alpha_tester_access.py" in value
    assert "--expected-candidate" in value
    assert "retention-days: 90" in value
    assert "does not enable public registration" in value
