...
WORKFLOW = (
    ...
    / ".github"
    / "workflows"
    / "incident-communication-readiness.yml"
)
...
def test_workflow_rejects_unsafe_paths_and_retains_report() -> None:
    ...
    assert "validate_incident_communication_readiness.py" in value
    assert "incident-communication-readiness-report.json" in value
    assert "retention-days: 90" in value
    assert "does not prove live status infrastructure" in value
