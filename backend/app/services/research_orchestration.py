...
class ResearchOrchestrator:
    ...
    def run(self, request: ResearchOrchestrationRequest, user_id: int) -> ResearchOrchestrationResponse:
        ...
        for step in plan:
            outputs.append(self._execute(step.role, request))
        ...
        run_id = sha256(
            f"{user_id}:{request.question.strip()}:{symbol or ''}:{len(outputs)}".encode()
        ).hexdigest()[:16]
        ...
        synthesis = ResearchSynthesis(
            conclusion=(
                ...
                "The current conclusion should remain provisional until the identified evidence gaps are resolved."
            ),
    ...
    def _confidence(self, request: ResearchOrchestrationRequest, gaps: list[str]) -> str:
