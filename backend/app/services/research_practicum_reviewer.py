...
def review_read(row: PracticumReviewDecision) -> PracticumReviewDecisionRead:
...
def readiness(db: Session, enrollment: PracticumEnrollment) -> PracticumReadinessRead:
    objectives = list(
        db.scalars(
    ...
    from app.schemas.research_practicum import PracticumObjectiveReadinessRead
    ...
    return PracticumReadinessRead(
        ...
        latest_review_decision=review_read(latest_review) if latest_review else None,
    )
...
def detail_enrollment(db: Session, enrollment_id: int) -> PracticumEnrollment:
...
def list_queue(
    ...
    *,
    ...
    if learner_query and (normalized_query := learner_query.strip().lower()):
...
def get_detail(db: Session, enrollment_id: int) -> PracticumReviewerDetailRead:
    enrollment = detail_enrollment(db, enrollment_id)
    ...
    return PracticumReviewerDetailRead(
        ...
        review_history=[review_read(row) for row in history],
        advisory_notice=ADVISORY_NOTICE,
    )
...
def record_decision(
    ...
    *,
    ...
    readiness_result = readiness(db, enrollment)
    if not readiness_result.ready_for_human_review:
    ...
    return get_detail(db, enrollment.id)
