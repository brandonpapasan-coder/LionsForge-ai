...
_PRIVATE_FIELDS = re.compile(
    ...
    r"reviewer[_-]?notes|answer[_-]?key|private[_-]?content|credential)", re.IGNORECASE,
)
...
def _window_row(start: datetime, end: datetime, entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "window_start": _utc_z(start), "window_end": _utc_z(end), "action_count": len(entries),
        "status_counts": status_counts, "completed_count": len(completed),
...
def build_trends(*, source_report: dict[str, Any], granularity: Literal["day", "week", "month"], range_start: datetime, range_end: datetime, generated_at: datetime) -> dict[str, Any]:
    if granularity not in SUPPORTED_GRANULARITIES:
        raise ValueError("unsupported granularity")
    requested_start = _utc(range_start)
    end = _utc(range_end)
    if end <= requested_start:
        raise ValueError("range_end must be after range_start")
    start = _floor_window(requested_start, granularity)
    ...
    while cursor < end:
        ...
        window_entries = [entry for entry in entries if cursor <= _parse_utc(entry["acted_at"]) < next_cursor]
        windows.append(_window_row(cursor, next_cursor, window_entries))
        cursor = next_cursor
    trends = {
        "schema": TREND_SCHEMA, "schema_version": SCHEMA_VERSION, "generator_version": GENERATOR_VERSION,
        "learner_user_id": source_report["learner_user_id"], "generated_at": _utc_z(generated_at),
        "source_report_sha256": sha256_digest(source_report),
        "source_excluded_record_count": source_report["excluded_record_count"],
        "granularity": granularity, "range_start": _utc_z(start), "range_end": _utc_z(end),
        "minimum_window_size": MIN_WINDOW_SIZE, "windows": windows, "advisory_notice": ADVISORY_NOTICE,
    }
...
def validate_trends(trends: Any) -> list[str]:
    ...
    required = {"schema", "schema_version", "generator_version", "learner_user_id", "generated_at", "source_report_sha256", "source_excluded_record_count", "granularity", "range_start", "range_end", "minimum_window_size", "windows", "advisory_notice"}
    ...
    if trends.get("schema") != TREND_SCHEMA: findings.append("unsupported trend schema")
    if trends.get("schema_version") != SCHEMA_VERSION or trends.get("generator_version") != GENERATOR_VERSION: findings.append("unsupported trend version")
    if trends.get("granularity") not in SUPPORTED_GRANULARITIES: findings.append("unsupported trend granularity")
    if trends.get("minimum_window_size") != MIN_WINDOW_SIZE: findings.append("minimum window size mismatch")
    if trends.get("advisory_notice") != ADVISORY_NOTICE: findings.append("trend advisory notice mismatch")
    if not isinstance(trends.get("source_report_sha256"), str) or not _DIGEST.fullmatch(trends["source_report_sha256"]): findings.append("source_report_sha256 must be a lowercase SHA-256 digest")
    ...
    if not isinstance(windows, list) or len(windows) > MAX_WINDOWS:
        ...
        for row in windows:
            if not isinstance(row, dict) or set(row) != expected:
                findings.append("window fields are invalid"); continue
            if previous_end is not None and row["window_start"] != previous_end: findings.append("windows are overlapping or non-contiguous")
            previous_end = row["window_end"]
            counts = row["status_counts"]
            if not isinstance(counts, dict) or tuple(counts) != _ALLOWED_STATUSES:
                findings.append("window status counts are invalid"); continue
            if any(not isinstance(value, int) or value < 0 for value in counts.values()): findings.append("window status counts are invalid")
            if sum(counts.values()) != row["action_count"]: findings.append("window status counts do not match action count")
            suppressed = row["action_count"] < MIN_WINDOW_SIZE
            if row["statistics_suppressed"] != suppressed: findings.append("window suppression mismatch")
            if suppressed and (row["completed_rate"] is not None or row["median_completion_hours"] is not None): findings.append("suppressed window statistics must be null")
    findings.extend(_scan_private_fields(trends))
    return sorted(set(findings))

def build_receipt(trends: dict[str, Any], *, generated_at: datetime) -> dict[str, Any]:
    findings = validate_trends(trends)
    if findings: raise ValueError("Cannot receipt invalid roadmap outcome trends: " + "; ".join(findings))
    return {"schema": RECEIPT_SCHEMA, "schema_version": SCHEMA_VERSION, "generator_version": GENERATOR_VERSION, "trends_sha256": sha256_digest(trends), "source_report_sha256": trends["source_report_sha256"], "window_count": len(trends["windows"]), "generated_at": _utc_z(generated_at)}

def validate_receipt(receipt: Any, trends: Any) -> list[str]:
    findings = validate_trends(trends)
    if not isinstance(receipt, dict): return sorted(set(findings + ["receipt must be an object"]))
    ...
    if receipt.get("schema") != RECEIPT_SCHEMA: findings.append("unsupported receipt schema")
    if isinstance(trends, dict):
        if receipt.get("trends_sha256") != sha256_digest(trends): findings.append("trends digest mismatch")
        if receipt.get("source_report_sha256") != trends.get("source_report_sha256"): findings.append("source report digest mismatch")
        if receipt.get("window_count") != len(trends.get("windows", [])): findings.append("window count mismatch")
